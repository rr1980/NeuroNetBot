...
public class Controller_6 : MonoBehaviour {
	...
	private void updateGeneration()
	{
		...
		if (RoundTime < (Time.realtimeSinceStartup - startTime))
		{
			...
			var bestNNs = bestGOs.Select(b => b.gameObject.GetComponent<NetworkController_6>()).ToList();
			...
			{
				var rrr = NNAnalyzer_6.Crossover(bestNNs[0].net, bestNNs[ind1].net, crossFactor, mutateChance, mutateDividor, bestNNs[0].Bias, antiInzest, SaveCrossDna);
				SaveCrossDna = false;
				if (rrr != null)
				{
					childs.AddRange(rrr);
			...
			{
				var chrnd = NNAnalyzer_6.Crossover(bestNNs[0].net, new NeuralNet(bestNNs[0].Bias, bestNNs[0].InpuNeuron, bestNNs[0].HiddenNeuron1, bestNNs[0].HiddenNeuron2,0, bestNNs[0].OutputNeuron), crossFactor, mutateChance, mutateDividor, bestNNs[0].Bias, antiInzest, SaveCrossDna);
	...
	private void buildGameObject(NeuralNet nn = null, int mark = 0)
	{
		...
		bot.SetActive(false);
		NetworkController_6 nc = bot.GetComponent<NetworkController_6>();
	...
	{
		return bots.OrderByDescending(b => b.gameObject.GetComponent<Bot_6>().Health).ToList();
	}
